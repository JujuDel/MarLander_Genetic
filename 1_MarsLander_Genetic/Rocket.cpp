...
Rocket::Rocket(const double f_x, const double f_y, const double f_vx,
               ...
               const std::int8_t f_power, const int f_fuel)
    : pX{f_x}, pY{f_y}, x{f_x}, y{f_y}, vx{f_vx}, vy{f_vy}, ax{f_power *
                                                               sin(-f_angle *
                                                                   _PI / 180.)},
      ay{f_power * cos(-f_angle * _PI / 180.) + _g}, angle{f_angle},
      power{f_power}, fuel{f_fuel}, isAlive{true}, floor_id_crash{-1} {}
...
void Rocket::updateRocket(const std::int8_t f_angle,
  ...
  if (fuel == 0) {
    ...
    power = std::min(static_cast<std::int8_t>(4),
                     std::max(static_cast<std::int8_t>(0),
                              static_cast<std::int8_t>(power + f_power)));
  }
...
void Rocket::debug(const double f_timeSec) const {
  ...
  std::cout << "    Angle=" << int(angle) << "deg, Power=" << int(power)
            << "m/s2" << std::endl;
  std::cout << std::endl;
}
